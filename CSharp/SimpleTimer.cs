...
namespace Benchmark
{
	...
	/// System.Diagnostics.Stopwatch. Its resolution is typically 15.6 ms.
	/// </summary>
	/// <remarks>
	/// With SimpleTimer, the timer starts when you construct the object and 
	/// it is always counting unless you call Pause(). You can get the elapsed 
	/// time and restart the timer from zero with a single call to Restart(). 
	/// The Stopwatch class requires you to make three separate method calls 
	/// to do the same thing: ElapsedMilliseconds, then Reset(), then Start().
	/// </remarks>
